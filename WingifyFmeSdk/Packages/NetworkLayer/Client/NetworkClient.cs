...
namespace WingifyFmeSdk.Packages.NetworkLayer.Client
{
    /// <summary>
    ...
    public class NetworkClient : NetworkClientInterface
    {
        ...
        static NetworkClient()
        {
            ...
            ServicePointManager.DefaultConnectionLimit = 200; // Allow concurrent POST requests without blocking
            ...
            for (int attempt = 0; attempt <= maxRetries; attempt++)
            {
                ...
                try
                {
                    ...
                    if (networkOptions.ContainsKey("headers"))
                    {
                        ...
                        foreach (KeyValuePair<string, string> header in headers)
                        {
                            ...
                            {
                                connection.ContentType = header.Value;
                            }
                            else if (header.Key.Equals("Content-Length", StringComparison.OrdinalIgnoreCase))
                            {
                                // Skip adding Content-Length to the headers collection; it is set via the property below
                            }
                            else
                            {
                    ...
                    connection.ContentLength = payloadBytes.Length;
                ...
                else
                {
                    ...
                    if (networkOptions.ContainsKey("path"))
                    {
                        ...
                        if (path.Contains(EventEnum.DEBUGGER_EVENT.GetValue()))
                        {
