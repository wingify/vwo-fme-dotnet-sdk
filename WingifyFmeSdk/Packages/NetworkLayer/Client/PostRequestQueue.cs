        ...
        private void HandleResponse(ResponseModel response, RequestModel request, ResponseDataForDebugging responseDataForDebugging)
        {
            try
            {
                ...
                if (response != null && response.GetTotalAttempts() > 0)
                {
                    ...
                    {
                        { "extraData", $"POST {SettingsManager.GetInstance().hostname + UrlService.GetEndpointWithCollectionPrefix(UrlEnum.EVENTS.GetUrl(), WingifyFmeSdk.Services.SettingsManager.GetInstance().isGatewayServiceProvided)}" },
